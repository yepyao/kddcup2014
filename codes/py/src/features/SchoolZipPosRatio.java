...
public class SchoolZipPosRatio {
	public static void main(String[] args) throws IOException{
		...
		while (s != null){
			ArrayList<String> splits = CSVFileUtil.fromCSVLinetoArray(s);
			if (kind.get(splits.get(8)) == null){
				kind.put(splits.get(8), 1);
				kindSum++;
			}
			projectTeacher.put(splits.get(0), splits.get(8));
			if (splits.size() > 33 && posH.get(splits.get(0)) != null){
				if (teacher.get(splits.get(8)) == null) teacher.put(splits.get(8), 1);
				else teacher.put(splits.get(8), teacher.get(splits.get(8))+1);
			}else if (splits.size() > 33){
				if (teacherAll.get(splits.get(8)) == null) teacherAll.put(splits.get(8), 1);
				else teacherAll.put(splits.get(8), teacherAll.get(splits.get(8))+1);
			}
			s = in.readLine();
		}
		in.close();
