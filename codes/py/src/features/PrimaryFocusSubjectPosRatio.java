...
public class PrimaryFocusSubjectPosRatio {
	public static void main(String[] args) throws IOException{
		...
		Hashtable<String, Integer> kind = new Hashtable<String, Integer>();
		int kindSum = 0;
		s  = in.readLine();
		s = in.readLine();
		while (s != null){
			ArrayList<String> splits = CSVFileUtil.fromCSVLinetoArray(s);
			if (kind.get(splits.get(21)) == null){
				kind.put(splits.get(21), 1);
				kindSum++;
			}
			projectTeacher.put(splits.get(0), splits.get(21));
			if (train.get(splits.get(0)) != null){
				if (splits.size() > 33 && posH.get(splits.get(0)) != null){
					if (teacher.get(splits.get(21)) == null) teacher.put(splits.get(21), 1);
					else teacher.put(splits.get(21), teacher.get(splits.get(21))+1);
				}else if (splits.size() > 33){
					if (teacherAll.get(splits.get(21)) == null) teacherAll.put(splits.get(21), 1);
					else teacherAll.put(splits.get(21), teacherAll.get(splits.get(21))+1);
				}
			}
			s = in.readLine();
		}
