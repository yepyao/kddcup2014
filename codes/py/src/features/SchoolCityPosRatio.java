...
public class SchoolCityPosRatio {
	public static void main(String[] args) throws IOException{
		...
		while (s != null){
			ArrayList<String> splits = CSVFileUtil.fromCSVLinetoArray(s);
			projectTeacher.put(splits.get(0), splits.get(6));
			if (splits.size() > 33 && posH.get(splits.get(0)) != null){
				if (teacher.get(splits.get(6)) == null) teacher.put(splits.get(6), 1);
				else teacher.put(splits.get(6), teacher.get(splits.get(6))+1);
			}else if (splits.size() > 33){
				if (teacherAll.get(splits.get(6)) == null) teacherAll.put(splits.get(6), 1);
				else teacherAll.put(splits.get(6), teacherAll.get(splits.get(6))+1);
			}
			s = in.readLine();
		}
		in.close();
