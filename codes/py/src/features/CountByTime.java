...
public class CountByTime {
	public static void main(String[] args) throws IOException, Exception{
		...
		while (s != null){
			...
			if (teacherInfo.get(teacher) != null){
				for (int i = teacherInfo.get(teacher).size()-1; i>=0; i--){
					...
					else if (posH.get(teacherInfo.get(teacher).get(i).projectID) != null) hit++;
					show++;
				}
			}
			out.write("2 0:"+String.valueOf(hit)+" 1:"+String.valueOf(Double.valueOf(hit)/show)+"\n");
			s = in.readLine();
		}
		in.close();
		out.close();
		...
		while (s != null){
			...
			if (teacherInfo.get(teacher) != null){
				for (int i = teacherInfo.get(teacher).size()-1; i>=0; i--){
					if (posH.get(teacherInfo.get(teacher).get(i).projectID) != null) hit++;
					show++;
				}
			}
			out.write("2 0:"+String.valueOf(hit)+" 1:"+String.valueOf(Double.valueOf(hit)/show)+"\n");
