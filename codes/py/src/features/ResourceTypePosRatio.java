...
public class ResourceTypePosRatio {
	public static void main(String[] args) throws IOException{
		...
		Hashtable<String, Integer> typeAll = new Hashtable<String, Integer>();
		...
		while (s != null){
			...
			}else if (splits.size() > 25){
				if (typeAll.get(splits.get(25)) == null) typeAll.put(splits.get(25), 1);
				else typeAll.put(splits.get(25), typeAll.get(splits.get(25))+1);
			}
			s = in.readLine();
		}
		in.close();
		...
		while (s != null){
			...
			String[] temp = s.split(" ");
			double ans = 0;
			if (type.get(projectType.get(id.get(temp[1]))) != null && typeAll.get(projectType.get(id.get(temp[1]))) != null)
				ans = (double)type.get(projectType.get(id.get(temp[1]))) / (double)typeAll.get(projectType.get(id.get(temp[1])));
			num.add(ans);
			avg += ans;
			s = in.readLine();
		}
