...
public class SchoolStatePosRatio {
	public static void main(String[] args) throws IOException{
		...
		while (s != null){
			ArrayList<String> splits = CSVFileUtil.fromCSVLinetoArray(s);
			projectTeacher.put(splits.get(0), splits.get(7));
			if (splits.size() > 33 && posH.get(splits.get(0)) != null){
				if (teacher.get(splits.get(7)) == null) teacher.put(splits.get(7), 1);
				else teacher.put(splits.get(7), teacher.get(splits.get(7))+1);
			}else if (splits.size() > 33){
				if (teacherAll.get(splits.get(7)) == null) teacherAll.put(splits.get(7), 1);
				else teacherAll.put(splits.get(7), teacherAll.get(splits.get(7))+1);
			}
			s = in.readLine();
		}
		in.close();
