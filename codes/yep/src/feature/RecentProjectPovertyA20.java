...
public class RecentProjectPovertyA20 extends FeatureList {
	...
	private void makeFeatureMap() throws Exception {
		...
		while (q < p_list.size()) {
			while (p < p_list.size()
				...
				if (p_list.get(p).poverty_level.equals("highest poverty")) sum++;
				p++;
			}
			while (q < p_list.size()
					&& (p >= p_list.size() || time_list.get(q)
							- time_list.get(p) >= days * 24 * 3600 * 1000)) {
				if (p_list.get(q).poverty_level.equals("highest poverty")) sum--;
				q++;
